...
void Graphics::DrawCircle(const glm::ivec2& point, int radius, const glm::vec4& color)
{
	...
	circleRGBA(m_renderer, point.x, point.y, radius, ConvertColor(color).r, ConvertColor(color).g, ConvertColor(color).b, ConvertColor(color).a);
}

void Graphics::DrawFilledCircle(const glm::ivec2& point, int radius, const glm::vec4& color)
{
	glm::ivec4 icolor = ConvertColor(color);
	filledCircleRGBA(m_renderer, point.x, point.y, radius, ConvertColor(color).r, ConvertColor(color).g, ConvertColor(color).b, ConvertColor(color).a);
}
